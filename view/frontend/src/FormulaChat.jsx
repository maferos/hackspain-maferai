...
// answers and, for a formula, proposes it against the flasks on the bench; Run
// hands it to the arm (formulation.py), and the side panels follow the run.
  ...
  return (
    <div className="proposal">
      ...
      <div className="proposal__foot">
        <span className="mono">
          {formula.targetMass.toFixed(3)} g · {formula.estimate.trips} draws · ~{clock(formula.estimate.seconds)}
        </span>
        {latest && ok.length > 0 ? (
          <button type="button" className="chat-button chat-button--run" disabled={running || busy} onClick={onRun}>
            Run
          </button>
        ) : null}
      </div>
...
// `run` is the LabState's run, to know whether a formulation is going.
export default function FormulaChat({ backendUrl, run, style }) {
  const [messages, setMessages] = useState([]);
  const [draft, setDraft] = useState("");
  const [busy, setBusy] = useState(false);
  const [info, setInfo] = useState(null);
  const logRef = useRef(null);
  const running = run?.status === "running" && !run?.scripted;
  ...
  const stop = async () => {
    try {
      ...
      say({ role: "assistant", text: "Stopping; the arm goes back to its sweep." });
    } catch (error) {
      say({ role: "assistant", text: error.message, error: true });
    }
  };
    ...
    >
      <ol className="chat__log" ref={logRef}>
        {messages.length === 0 ? (
          <li className="chat__hint">
            Type a formula and the arm pipettes it into the beaker on the balance.
            {info && !info.available ? " This scene has no pipette: run the backend with the rail scene." : ""}
          </li>
        ) : null}
        {messages.map((m, i) => (
      ...
      >
        <input
          ...
          placeholder={running ? "Type stop, or the next formula…" : "e.g. 1.2 g geraniol, 0.5 g nerol"}
