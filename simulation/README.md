...
| `scripts/generate_iris_pipette_scene.py` | Writes `assets/ur10e_iris_pipette/` (attachable MJCF, the 60 ml amber bottle and its PP25 cap, meshes, URDFs) and `models/iris_pipette_scene.xml` |
...
| `assets/labelled_bottles/` | All 200 catalogued samples as MuJoCo bodies, each bottle carrying a ring of eight ArUco markers as a texture (`marker_id` in the lookup table; it was an EAN-13 label before), and the same marker once more on top of its cap (built by `tools/build_labelled_bottles.py`). These are what the scene uses |
