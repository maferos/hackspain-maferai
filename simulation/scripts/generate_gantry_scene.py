...
def build_scene() -> Path:
    ...
    # The four ceiling extraction assemblies intersect the gantry workspace.
    # Remove the full assemblies from a room asset beside the source, keeping
    # its relative mesh paths valid and the arm scene's room intact.
    room_model = assets.find('model[@name="lab_room"]')
    room_source = (OUT.parent / room_model.get('file')).resolve()
    room_root = ET.parse(room_source).getroot()
    for parent in room_root.iter():
        for geom in list(parent.findall('geom')):
            if geom.get('name', '').startswith('snorkel_'):
                parent.remove(geom)
    room_out = room_source.with_name('lab_room_gantry.xml')
    ET.indent(room_root, space='  ')
    room_out.write_text('<?xml version="1.0" encoding="utf-8"?>\n'
                       '<!-- Generated by scripts/generate_gantry_scene.py. -->\n'
                       + ET.tostring(room_root, encoding='unicode') + '\n')
    room_model.set('file', '../' + room_out.relative_to(SIM).as_posix())
