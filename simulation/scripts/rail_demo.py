...
def trajectory(model: mujoco.MjModel, waypoints: list[tuple[float, np.ndarray]],
    ...
    """
    rows = []
    for (x0, q0), (x1, q1) in itertools.pairwise(waypoints):
        seconds = max(abs(x1 - x0) / speed,
                      float(np.abs(q1 - q0).max()) / 1.4, 0.35)
        for step in range(max(int(seconds * FPS), 1)):
            alpha = (step + 1) / max(int(seconds * FPS), 1)
...
def main() -> None:
    ...
    parser.add_argument('--mode', choices=('sweep', 'visit'), default='sweep')
    parser.add_argument('--visits', type=int, default=6,
                        help='vessels to scan in visit mode')
    ...
    builder = (sweep_waypoints(model, data) if args.mode == 'sweep'
               else visit_waypoints(model, data, args.visits))
    rows = trajectory(model, builder, args.speed)
