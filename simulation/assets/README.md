...
| [`amber_bottles/`](amber_bottles/) | **Yes** — `amber_060ml.xml`, `amber_100ml.xml` | Via the source `.glb` (asset importer) | From `../../assets/amber-bottles/glb/` by `tools/convert_glb.py`. Hollow glass: the converter sets `inertia="exact"` (65 g / 96 g; MuJoCo's default gives 446 g for the 60 ml). Mesh geoms collide (convex hull). Loose, movable bottles in the scene; dropped and settled upright. Verified. |
| [`reagent_jars/`](reagent_jars/), [`agro_bottles/`](agro_bottles/) | **Yes** — one XML per size | Via the source `.glb` | HDPE powder jars (250/500/1000 ml) and white HDPE bottles + caps (500 ml, 1 l, 2 l), from `../../assets/reagent-bottles/` and `../../assets/agrochemical-bottles/` by `tools/convert_glb.py`. Label textures are dropped (labels render white). Placed in the scene's entrance corner. Verified. |
